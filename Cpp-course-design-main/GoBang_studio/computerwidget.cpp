...
computerWidget::computerWidget(menu* menuptr, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::computerWidget)
{
    ...
    connect(timer, &QTimer::timeout, this, &computerWidget::on_forwardBtn_clicked);
}
...
void computerWidget::on_returnBtn_clicked()
{
    this->closeEvent(new QCloseEvent);
}

void computerWidget::paintEvent(QPaintEvent *event) {
    // 绘制棋盘
...
void computerWidget::mouseMoveEvent(QMouseEvent* event)
{
    // 人机对战时，回看前几步的时候，不能落子（和联系模式不同）
...
void computerWidget::mousePressEvent(QMouseEvent *event)
{
    // 鼠标左键，点击落子
...
//    update();
    ...
    // 落子：1更新cb；2更新tablewidget，加上一行
    currentRow = QString(bestMove->x + 'A');
    currentCol = QString::number(bestMove->y + 1);
    currentMove = currentRow + currentCol;

    cb->addMove(bestMove->x, bestMove->y);
...
//    int rating = (bestMove->score) * (bestMove->color ? 1 : -1);
...
void computerWidget::closeEvent(QCloseEvent *event)
{
    if (!closing) {
        if (need_to_save) {
...
void computerWidget::on_openBtn_clicked()
{
    // 读取棋谱
    if (need_to_save) {
        QMessageBox::StandardButton resBtn = QMessageBox::question(this, "保存棋谱提示",
...
void computerWidget::on_saveBtn_clicked()
{
    // 保存棋谱
    // 定义文件路径
    ...
    QString filePath = folderPath + "/" + dateTime + "_computer.txt";
...
void computerWidget::on_tableWidget_cellClicked(int row, int column)
{
    // 点击手顺表格格子
    // 显示到这一步的手顺的棋子
...
void computerWidget::on_forwardBtn_clicked()
{
    // 后一个手顺
    if (show_piece_num == cb->moveCount()) {
        timer->stop();
...
void computerWidget::on_superForwardBtn_clicked()
{
...
void computerWidget::on_backwardBtn_clicked()
{
    // 前一个手顺
...
void computerWidget::on_superBackwardBtn_clicked()
{
    // 最前一个手顺
    if (cb->moveCount() == 0 || show_piece_num == 0 || show_piece_num == 1) {
        return;
...
void computerWidget::on_playBtn_clicked()
{
    // 从当前手顺开始播放
...
void computerWidget::on_regretBtn_clicked()
{
    // 悔棋一下子悔棋两步！
    if (cb->moveCount() <= 1) {
        QMessageBox::information(this, "悔棋提示", "已是最前一步，不能再悔棋！");
        return;
    }

    cb = cb->getIthSlice(cb->moveCount() - 2);
...
void computerWidget::on_startBtn_clicked()
{
    // 重新开始游戏
    // 有机会可以加上提示保存的逻辑
    cb = new chessboard;
    ...
    if (ui->comboBox->currentIndex() == 1) {
        ...
        // 落子：1更新cb；2更新tablewidget，加上一行
        int default_row, default_col;
        default_row = default_col = 7;
        QString currentRow = QString(default_row + 'A');
        QString currentCol = QString::number(default_col + 1);
        QString currentMove = currentRow + currentCol;

        cb->addMove(default_row, default_col);

        ui->tableWidget->setRowCount(cb->moveCount());

        show_piece_num = cb->moveCount();

        qDebug() << "In file computerWidget.cpp: on_startBtn_clicked(): addMove: "<< currentMove <<"\n";
        ...
        // 清除当前选择，使得右方表格选中最下面那个格子
        ui->tableWidget->selectionModel()->clearSelection();
        ui->tableWidget->selectionModel()->select(ui->tableWidget->model()->index(0, 0),
                                                 QItemSelectionModel::Select | QItemSelectionModel::Rows);
